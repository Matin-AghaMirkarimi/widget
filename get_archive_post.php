...
class get_archive_post extends \Elementor\Widget_Base {
	...
	protected function render() {
		...
		if ( ! isset ( $_GET['action'] ) && $_GET['action'] != 'elementor' ) {
			$all_date_search = get_beetween_date( $in_date, $out_date );
		...
		?>
		<?php
		if ( $terms_object->taxonomy == 'city' ) {
			?>
            <h1 class="fz14 fw500 mbt10">اجاره ویلاوسوییت در <span class="fz16 fw600"><?php echo $terms_object->name ?></span></h1>
                        ...
                        <div class="archive_post_item">
                            ...
                            <a href="<?php echo get_the_permalink( $row->ID ) ?>">
								...
								if ( $meta[0]['off_price'] != '' ) {
									?>
                                    <div class="mbt10">
                                        <span class="archive_discount fz10 fw300  fw400">تا 10 درصد تخفیف</span>
                                    </div>

								<?php } else {
									?>
                                    <div class="mbt10 width100 mr12"><a href="<?php echo get_the_permalink( $row->ID ) ?>" class="archive_view_pack fz10 fw300 col_gray2 fw400">مشاهده قیمت و پکیج های قابل رزرو</a></div>
								<?php }
//							echo 	print_r($meta);
								?>
                                <span class=" fz10 fw400 col_gray2 mr5 mbt10 width100 mr12 ">شروع قیمت : <span class="fz12 fw700">&nbsp;<?php echo number_format( $meta['price'] ) ?>&nbsp;تومان</span> / <span class="col_gray">هر شب</span></span>
	                            <?php
					...
					foreach ( $posts as $row ) {
						...
						$reserved      = [];
						$check_reserve = 0;
						if ( in_array( $all_date_search, $reserved ) ) {
							$check_reserve = 1;
						}
						if ( $meta[0]['total_capacity'] >= $cap && $check_reserve == 0 ) {
                            ...
                            <div class="archive_post_item">
                                ...
                                <a href="<?php echo get_the_permalink( $row->ID ) ?>" class="pos_relative">
									...
									<?php } else {
										?>
                                        <div class="mbt10"><a href="<?php echo get_the_permalink( $row->ID ) ?>" class="archive_view_pack fz10 fw300 col_gray2 fw400">مشاهده قیمت و پکیج های قابل رزرو</a></div>
									<?php }
									?>
                                    <span class=" fz10 fw400 col_gray2 mr5 mbt10 mr5">شروع قیمت : <span class="fz12 fw700">&nbsp;<?php echo number_format( $meta[0]['price'] ) ?>&nbsp;تومان</span> / <span class="col_gray">هر شب</span></span>
