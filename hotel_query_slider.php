...
class hotel_query_slider extends \Elementor\Widget_Base {
	...
	protected function render() {
	 ...
	 $show_id    = $values['hqs_shows_id'];
		...
		$t_query = [];
		if ( $CatQuery != '' || $cityQuery != '' || $regionQuery != '' || $selectedQuery != '' ) {
			...
			if ( $cityQuery[0]['hqs_SliderQueryCity'] != '' ) {
				...
				$end_city_array = [ 'taxonomy' => 'city_hotel', 'field' => 'slug', 'terms' => $city_array ];
			...
			if ( $CatQuery[0]['hqs_SliderQueryCat'] != '' ) {
				...
				$end_cat_array = [ 'taxonomy' => 'hotel_category', 'field' => 'slug', 'terms' => $cat_array ];
			...
			if ( $toolsQuery[0]['hqs_SliderQueryTools'] != '' ) {
				...
				$end_tools_array = [ 'taxonomy' => 'hotel_tools', 'field' => 'slug', 'terms' => $tools_array ];
            ...
            <div class="swiper query_slider<?php echo $rand ?>">
                ...
                <div class="swiper-wrapper">
					...
					foreach ( $the_query->posts as $row ) {
						...
						$item_city      = get_the_terms( $row->ID, 'city_hotel' );
						$image          = wp_get_attachment_image_src( get_post_thumbnail_id( $row->ID ), array( 300, 166 ), true );
...
//						}
                        ...
                        <div class="swiper-slide">
                            <a href="<?php echo get_the_permalink( $row->ID ) ?>">
                               ...
                               <?php     }
                                    ?>
                                    <span class="n_span ml_10 padd10"><?php echo $row->post_title ?></span>
